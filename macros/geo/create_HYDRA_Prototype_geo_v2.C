...
void create_tpc_geo(string geoTag = "Prototype") {
  ...
  TString geoFileManName = geoFileName + geoTag.c_str() + ".geoMan_v2.root";
  geoFileName = geoFileName + geoTag.c_str() + ".geo_v2.root";
  ...
  // Global definition of TPC position in cm
 TVector3 gTrans(8.6, 0, 271); // HARD-CODED TODO create parameters TPCCenterX,TPCCenterY, TPCCenterZ
...
void ConstructTPC(TGeoVolume *pWorld) {
  ...
  TGeoShape *frame_window =
      new TGeoBBox("frame_window", 2 * FrameThickness, Windowy, Windowz);
  ...
  solidFrame = new TGeoCompositeShape(
      "Frame", "frame_box -frame_window:tc1 -frame_window:tc2");
  logicFrame = new TGeoVolume("Frame", solidFrame, FrameMaterial);
  // Gas region---------------------------------------------------------------

  solidGas =
      new TGeoBBox("gas_box", TPCLx - 2 * FrameThickness,
                   TPCLy - 2 * FrameThickness, TPCLz - 2 * FrameThickness);
  logicGas = new TGeoVolume("gas_box", solidGas, GasMaterial);
  // Active region------------------------------------------------------------

  solidActiveRegion = new TGeoBBox("Active_region", ActiveRegionx,
                                   ActiveRegiony, ActiveRegionz);
  logicActiveRegion =
      new TGeoVolume("Active_region", solidActiveRegion, GasMaterial);

  TGeoTranslation *tc3 = new TGeoTranslation(
      "tc3", 0, 0,
  ...
  // front_window
  logicFrame->AddNode(logicFWindow, 0,
                      new TGeoCombiTrans(-(TPCLx - 2 * FrameThickness + 0.0025),
                                         0, 0, zeroRot));
  // back_window
  logicFrame->AddNode(
      logicBWindow, 0,
      new TGeoCombiTrans(TPCLx - 2 * FrameThickness + 0.0025, 0, 0, zeroRot));
  // GAS
