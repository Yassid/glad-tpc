                 ...
                 bool useXtr = true)
{
    ...
    for (Long64_t i = 0; i < std::min(tS->GetEntries(), tU->GetEntries()); ++i) {
        ...
        // Get chord from the UKF smoothed positions (already in cm, local
        // frame after R3BGTPCFitterUKF's mm→cm rescale).
